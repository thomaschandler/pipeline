...
func exampleTest(path string, waitValidateFunc waitFunc, kind string) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// Same check as in github.com/tektoncd/plumbing/scripts/e2e-tests.sh
		_, err := c.RbacV1Client.ClusterRoleBindings().Get("cluster-admin-binding", metav1.GetOptions{})
		if err != nil {
			t.Logf("cluster-admin-binding result: %s", err)
			adminAvailable = false
		}

		// If cluster-admin-binding is available, use it as CI is running, and
		// admin is required for some examples
		if adminAvailable {
			t.Log("Using cluster-admin user for YAML tests")
			adminOverride := &clientcmd.ConfigOverrides{
				Context: api.Context{
					AuthInfo: "cluster-admin",
				},
			}

			c, namespace = setupWithOverrides(t, adminOverride)
		}
